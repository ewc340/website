...
> decisions lives in [`BUILD_PLAN.md`](./BUILD_PLAN.md), and the detailed,
> step-by-step deployment instructions live in
> [`DEPLOYMENT.md`](./DEPLOYMENT.md) — those are the documents to read if
> you want to know precisely how this will get built and shipped, before
> any code is written.
...
| Topic | Decision |
|---|---|
| Homepage layout | **Option D — Editorial / Magazine**, chosen (superseded an earlier Option C pick) |
...
| Résumé | **No dedicated page.** A "Résumé" link/button lives on the homepage (in the side-rail author card) pointing to a PDF, opened in a new tab |
...
**Option D (Editorial / Magazine) is the chosen direction** — kept here for
the full record, along with the others for context on why.
...
### Layout D — Editorial / Magazine (blog-forward) ✅ **Chosen**
...
posts, a slim "About the author + tags + featured projects" rail. Adapted
final structure (no Research/Teaching/Experience, résumé as a link not a
page, newsletter signup deferred to the Phase 2 blog roadmap rather than
launch — see `BUILD_PLAN.md` §7 for the exact section-by-section spec):
**hero headline, one featured-post card, a grid of recent posts, and a side
rail with an About-the-author mini card, popular tags, and a featured-
projects mini list.**
...
- **Trade-off:** if you don't post consistently, an empty/sparse blog feed
  is the first thing visitors see — mitigated by seeding 2–3 real posts
  before launch (Phase 6 in `BUILD_PLAN.md` §9) so it never looks empty.

### Decision

**Option D, chosen.** The homepage *is* the blog-forward editorial landing
described above, and it inherits the **clickable, sticky in-page section
navigation** from the blog post mockup (§3) on every individual post page —
that feature is a property of the post-reading page itself, not the
homepage shell, so it applies no matter what wraps around it. The side
rail keeps the portfolio (About/projects) visible without competing with
writing for top billing, which matches "professional first, but the blog is
the growing second pillar."
...
| Concern | Choice | Why |
|---|---|---|
| Framework | **Next.js (App Router, latest stable — 16 as of this writing) + TypeScript** | SSG for portfolio pages, SSR/ISR-ready for blog, file-based routing, best-in-class SEO/metadata + OG image APIs, huge ecosystem, same family as the reference site so it's a proven fit for this genre of site |
...
| Fonts | UI/nav: **Geist Sans** (or Inter). Homepage headline + blog post titles/body: a serif display face (**Newsreader**, **Lora**, or **Fraunces**) | Layout D leans on a serif editorial headline as its signature visual move — this pairing is now load-bearing for the design, not optional |
...
The featured-post card, recent-posts grid, and side-rail widgets all need to
look consistent with each other since Option D surfaces several UI patterns
at once on the homepage — see `BUILD_PLAN.md` §5 for the concrete "design
once, reuse everywhere" component list this implies.
...
**not** block building or even shipping the site — summary below; the full
click-by-click steps (GitHub → Vercel import, env vars, exact DNS records,
post-launch checklist) are in [`DEPLOYMENT.md`](./DEPLOYMENT.md).
...
4. Once purchased (suggest Cloudflare Registrar for no markup, or
   Namecheap): point DNS at Vercel (A record for the apex + CNAME for
   `www`, exact values in `DEPLOYMENT.md` §5), Vercel auto-provisions
   HTTPS, update `NEXT_PUBLIC_SITE_URL`, redeploy. No downtime, no
   re-architecture.
...
2. Build the Home editorial layout (featured post + recent-posts grid +
   side rail), Projects section, and the Blog engine (MDX pipeline with
   footnotes/citations/TOC) in parallel-friendly slices.
